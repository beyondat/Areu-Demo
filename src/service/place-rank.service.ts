...
@Injectable()
export class PlaceRankService{
  getRankItems(): PlaceRank[] {
    return placaRankItems.map(p => new PlaceRank(p.id, p.title, p.rating, p.description, p.img));
  }

  getRankItem(id: number): PlaceRank {
    return placaRankItems.find(item => item.id === id);
  }
}
